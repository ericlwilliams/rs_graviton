...
bool calcSystematics::LoadSystematicsFiles(TObjArray* sys_files,vector<string>* sys_names){
  ...
  //m500
  string kkg_m500_name = "afii.kkg.lvjj.m500";
  sys_names->push_back(kkg_m500_name);
    
  string kkg_m500_file_name =
    file_loc+"mc.afii.kkg.lvjj.m500.systematics.plot.root";
  TFile* kkg_m500_file = TFile::Open(kkg_m500_file_name.c_str());
  sys_files->Add(kkg_m500_file);

  //m600
  string kkg_m600_name = "afii.kkg.lvjj.m600";
  sys_names->push_back(kkg_m600_name);
    
  string kkg_m600_file_name =
    file_loc+"mc.afii.kkg.lvjj.m600.systematics.plot.root";
  TFile* kkg_m600_file = TFile::Open(kkg_m600_file_name.c_str());
  sys_files->Add(kkg_m600_file);

  //m700
  string kkg_m700_name = "afii.kkg.lvjj.m700";
  sys_names->push_back(kkg_m700_name);
    
  string kkg_m700_file_name =
    file_loc+"mc.afii.kkg.lvjj.m700.systematics.plot.root";
  TFile* kkg_m700_file = TFile::Open(kkg_m700_file_name.c_str());
  sys_files->Add(kkg_m700_file);

  //m800
  string kkg_m800_name = "afii.kkg.lvjj.m800";
  sys_names->push_back(kkg_m800_name);
    
  string kkg_m800_file_name =
    file_loc+"mc.afii.kkg.lvjj.m800.systematics.plot.root";
  TFile* kkg_m800_file = TFile::Open(kkg_m800_file_name.c_str());
  sys_files->Add(kkg_m800_file);


  //m900
  string kkg_m900_name = "afii.kkg.lvjj.m900";
  sys_names->push_back(kkg_m900_name);
    
  string kkg_m900_file_name =
    file_loc+"mc.afii.kkg.lvjj.m900.systematics.plot.root";
  TFile* kkg_m900_file = TFile::Open(kkg_m900_file_name.c_str());
  sys_files->Add(kkg_m900_file);

  //m1000
  string kkg_m1000_name = "afii.kkg.lvjj.m1000";
  sys_names->push_back(kkg_m1000_name);
    
  string kkg_m1000_file_name =
    file_loc+"mc.afii.kkg.lvjj.m1000.systematics.plot.root";
  TFile* kkg_m1000_file = TFile::Open(kkg_m1000_file_name.c_str());
  sys_files->Add(kkg_m1000_file);

  //m1100
  string kkg_m1100_name = "afii.kkg.lvjj.m1100";
  sys_names->push_back(kkg_m1100_name);
    
  string kkg_m1100_file_name =
    file_loc+"mc.afii.kkg.lvjj.m1100.systematics.plot.root";
  TFile* kkg_m1100_file = TFile::Open(kkg_m1100_file_name.c_str());
  sys_files->Add(kkg_m1100_file);

  //m1200
  string kkg_m1200_name = "afii.kkg.lvjj.m1200";
  sys_names->push_back(kkg_m1200_name);
    
  string kkg_m1200_file_name =
    file_loc+"mc.afii.kkg.lvjj.m1200.systematics.plot.root";
  TFile* kkg_m1200_file = TFile::Open(kkg_m1200_file_name.c_str());
  sys_files->Add(kkg_m1200_file);

  //m1300
  string kkg_m1300_name = "afii.kkg.lvjj.m1300";
  sys_names->push_back(kkg_m1300_name);
    
  string kkg_m1300_file_name =
    file_loc+"mc.afii.kkg.lvjj.m1300.systematics.plot.root";
  TFile* kkg_m1300_file = TFile::Open(kkg_m1300_file_name.c_str());
  sys_files->Add(kkg_m1300_file);

  //m1400
  string kkg_m1400_name = "afii.kkg.lvjj.m1400";
  sys_names->push_back(kkg_m1400_name);
    
  string kkg_m1400_file_name =
    file_loc+"mc.afii.kkg.lvjj.m1400.systematics.plot.root";
  TFile* kkg_m1400_file = TFile::Open(kkg_m1400_file_name.c_str());
  sys_files->Add(kkg_m1400_file);

  //m1500
  string kkg_m1500_name = "afii.kkg.lvjj.m1500";
  sys_names->push_back(kkg_m1500_name);
    
  string kkg_m1500_file_name =
    file_loc+"mc.afii.kkg.lvjj.m1500.systematics.plot.root";
  TFile* kkg_m1500_file = TFile::Open(kkg_m1500_file_name.c_str());
  sys_files->Add(kkg_m1500_file);
...
void calcSystematics::MakePlot(string sample_name,string systematic_name,string var_name,string proc_name){
  ...
  for(int m=0;m<nMods;m++){
    ...
    if(systematic_name.find("vjets")==string::npos){
    if(hists[m]->GetRMS() < max_width){
      max_mod=m;
      //max_integral = hists[m]->Integral();
      max_width = hists[m]->GetRMS();
    }
    if(!hists[m]){
  ...
  hists[nDown]->GetXaxis()->SetTitleSize(0.06);
  hists[nUp]->GetXaxis()->SetTitleSize(0.06);
  hists[nNom]->GetXaxis()->SetTitleSize(0.06);

  hists[nDown]->GetXaxis()->SetTitleOffset(1);
  hists[nUp]->GetXaxis()->SetTitleOffset(1);
  hists[nNom]->GetXaxis()->SetTitleOffset(1);

  hists[nDown]->GetYaxis()->SetTitle("Events");
  hists[nUp]->GetYaxis()->SetTitle("Events");
  hists[nNom]->GetYaxis()->SetTitle("Events");

  hists[nDown]->GetYaxis()->SetTitleOffset(0.8);
  hists[nUp]->GetYaxis()->SetTitleOffset(0.8);
  hists[nNom]->GetYaxis()->SetTitleOffset(0.8);
  ...
  TLegend* plot_legend = new TLegend(0.7,0.65,0.9,0.90,systematic_name.c_str());
  plot_legend->SetFillColor(0);
  plot_legend->SetBorderSize(0);
  plot_legend->SetTextSize(0.045);

  char lgndNameUp[100];
  sprintf(lgndNameUp,"%s; %.1f","up",hists[nUp]->Integral());
  plot_legend->AddEntry(hists[nUp],lgndNameUp,"F");
  char lgndNameNom[100];
  sprintf(lgndNameNom,"%s; %.1f","nom",hists[nNom]->Integral());
  plot_legend->AddEntry(hists[nNom],lgndNameNom,"F");
  char lgndNameDown[100];
  sprintf(lgndNameDown,"%s; %.1f","down",hists[nDown]->Integral());
...
void calcSystematics::PlotBackgrounds(string cur_sys,string cur_proc,string var_name,int n_bkgds,string bkgds[]){
  ...
  TPad* ratio_pad =  new TPad("ratio_pad","ratio_pad",0.01,0.01,0.99,ratio_split);
  TPad* plot_pad = new TPad("plot_pad","plot_pad",0.01,ratio_split,0.99,0.99);
  ...
  int n_bins = ((TAxis*) bkgd_nom_hist->GetXaxis())->GetNbins();
  float x_min = ((TAxis*) bkgd_nom_hist->GetXaxis())->GetXmin();
  float x_max = ((TAxis*) bkgd_nom_hist->GetXaxis())->GetXmax();

  string hist_ratio_up_name = "h_"+cur_proc+"_"+cur_sys+"_ratio_up_"+var_name;
  //TH1F* hist_ratio_up = new TH1F(hist_ratio_up_name.c_str(),"",n_bins,x_min,x_max);
  TH1F* hist_ratio_up = (TH1F*) bkgd_up_hist->Clone();
  hist_ratio_up->Add(bkgd_nom_hist,-1.);
  hist_ratio_up->Divide(hist_ratio_up,bkgd_nom_hist,1,1,"B");
  
  string hist_ratio_down_name = "h_"+cur_proc+"_"+cur_sys+"_ratio_down_"+var_name;
  //TH1F* hist_ratio_down = new TH1F(hist_ratio_down_name.c_str(),"",n_bins,x_min,x_max);
  TH1F* hist_ratio_down = (TH1F*) bkgd_down_hist->Clone();
  hist_ratio_down->Add(bkgd_nom_hist,-1.);
  hist_ratio_down->Divide(hist_ratio_down,bkgd_nom_hist,1,1,"B");
  ...
  TLegend* plot_legend = new TLegend(0.7,0.65,0.9,0.9,cur_sys.c_str());
  plot_legend->SetFillColor(0);
  plot_legend->SetBorderSize(0);
  plot_legend->SetTextSize(0.045);

  char lgndNameUp[100];
  sprintf(lgndNameUp,"%s; %.1f","up",bkgd_up_hist->Integral());
  plot_legend->AddEntry(bkgd_up_hist,lgndNameUp,"F");
  char lgndNameNom[100];
  sprintf(lgndNameNom,"%s; %.1f","nom",bkgd_nom_hist->Integral());
  plot_legend->AddEntry(bkgd_nom_hist,lgndNameNom,"F");
  char lgndNameDown[100];
  sprintf(lgndNameDown,"%s; %.1f","down",bkgd_down_hist->Integral());
  ...
  plot_legend->Draw();
	...
	TAxis* ratio_up_xaxis= hist_ratio_up->GetXaxis();
	TAxis* ratio_up_yaxis= hist_ratio_up->GetYaxis();

	hist_ratio_up->SetMarkerStyle(20);
	hist_ratio_up->SetMarkerSize(2);
	hist_ratio_up->SetLineWidth(1);
	hist_ratio_up->SetLineColor(kRed);

	ratio_up_xaxis->SetLabelSize(0.08);
	ratio_up_yaxis->SetLabelSize(0.08);
    
	ratio_up_xaxis->SetTitle("M(lvjj) [GeV]");
	ratio_up_xaxis->SetTitleSize(0.16);
	ratio_up_xaxis->SetTitleOffset(0.7);
	ratio_up_yaxis->SetNdivisions(4,4,4);
	ratio_up_yaxis->SetTitle("(mod-nom)/nom");
	ratio_up_yaxis->SetTitleSize(0.07);
	ratio_up_yaxis->SetTitleOffset(0.36);

	ratio_up_yaxis->SetRangeUser(-0.5,0.5);
	hist_ratio_up->Draw("PE");
	hist_canvas->Update();
	
	TAxis* ratio_down_yaxis= hist_ratio_down->GetYaxis();
	hist_ratio_down->SetMarkerStyle(20);
	hist_ratio_down->SetMarkerSize(2);
	hist_ratio_down->SetLineColor(kBlue);
	ratio_down_yaxis->SetRangeUser(-0.5,0.5);
	hist_ratio_down->Draw("PE sames");
	hist_canvas->Update();


	plot_pad->cd();
	plot_pad->SetLogy();
	hist_canvas->RedrawAxis();
	hist_canvas->Update();
