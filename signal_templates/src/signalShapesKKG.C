#define signalShapesKKG_cxx
#include "signalShapesKKG.h"
...
const int n_sig_masses=11;//500-1500; 100 GeV steps
const int signal_masses_int[n_sig_masses] = {500,600,700,800,900,1000,1100,1200,1300,1400,1500};

const int NGmass  = 31;
const int GmassInt[NGmass] =
...
const double signal_evt_n = 14996;

signalShapesKKG::signalShapesKKG(){
...
void signalShapesKKG::makeSignalParameterFits(string channel/*="evjj"*/,string cur_sys/*="jes_nom"*/){
  //gROOT->SetBatch();
  ...
  string signal_file_name = file_loc+"mc.afii.kkg.lvjj.m%i.systematics.plot.root";

  lvjjHelper::csHelper* gww_cs_helper = new lvjjHelper::csHelper();
  
  double kkg_xsecs[NGmass]={0.0};
  for(int i=0;i<NGmass;i++){//0.8 - flat filter eff, (4/9) BR(WW->lvjj)
    kkg_xsecs[i] = (0.8)*(4./9)*gww_cs_helper->BulkRSGravitonWWCS((double) GmassInt[i]*1.0);
  }

  delete gww_cs_helper;
  ...
  Double_t CrossSection_m500 = kkg_xsecs[0];
  Double_t CrossSection_m550 = kkg_xsecs[1];
  Double_t CrossSection_m600 = kkg_xsecs[2];
  Double_t CrossSection_m650 = kkg_xsecs[3];
  Double_t CrossSection_m700 = kkg_xsecs[4];
  Double_t CrossSection_m750 = kkg_xsecs[5];
  Double_t CrossSection_m800 = kkg_xsecs[6];
  Double_t CrossSection_m850 = kkg_xsecs[7];
  Double_t CrossSection_m900 = kkg_xsecs[8];
  Double_t CrossSection_m950 = kkg_xsecs[9];
  Double_t CrossSection_m1000 = kkg_xsecs[10];
  Double_t CrossSection_m1050 = kkg_xsecs[11];
  Double_t CrossSection_m1100 = kkg_xsecs[12];
  Double_t CrossSection_m1150 = kkg_xsecs[13];
  Double_t CrossSection_m1200 = kkg_xsecs[14];
  Double_t CrossSection_m1250 = kkg_xsecs[15];
  Double_t CrossSection_m1300 = kkg_xsecs[16];
  Double_t CrossSection_m1350 = kkg_xsecs[17];
  Double_t CrossSection_m1400 = kkg_xsecs[18];
  Double_t CrossSection_m1450 = kkg_xsecs[19];
  Double_t CrossSection_m1500 = kkg_xsecs[20];
  Double_t CrossSection_m1550 = kkg_xsecs[21];
  Double_t CrossSection_m1600 = kkg_xsecs[22];
  Double_t CrossSection_m1650 = kkg_xsecs[23];
  Double_t CrossSection_m1700 = kkg_xsecs[24];
  Double_t CrossSection_m1750 = kkg_xsecs[25];
  Double_t CrossSection_m1800 = kkg_xsecs[26];
  Double_t CrossSection_m1850 = kkg_xsecs[27];
  Double_t CrossSection_m1900 = kkg_xsecs[28];
  Double_t CrossSection_m1950 = kkg_xsecs[29];
  Double_t CrossSection_m2000 = kkg_xsecs[30];
 ...
 RooRealVar cb_a4("cb_a4","Delimiting Value", 1.0, 0.01, 50.0);
  ...
  // N Events expected: xsec*Lumi*acceptance
  Total_Channel_m500 = Total_Lumi*kkg_xsecs[0]*Acceptance_m500;
  Total_Channel_m600 = Total_Lumi*kkg_xsecs[2]*Acceptance_m600;
  Total_Channel_m700 = Total_Lumi*kkg_xsecs[4]*Acceptance_m700;
  Total_Channel_m800 = Total_Lumi*kkg_xsecs[6]*Acceptance_m800;
  Total_Channel_m900 = Total_Lumi*kkg_xsecs[8]*Acceptance_m900;
  Total_Channel_m1000 = Total_Lumi*kkg_xsecs[10]*Acceptance_m1000;
  Total_Channel_m1100 = Total_Lumi*kkg_xsecs[12]*Acceptance_m1100;
  Total_Channel_m1200 = Total_Lumi*kkg_xsecs[14]*Acceptance_m1200;
  Total_Channel_m1300 = Total_Lumi*kkg_xsecs[16]*Acceptance_m1300;
  Total_Channel_m1400 = Total_Lumi*kkg_xsecs[18]*Acceptance_m1400;
  Total_Channel_m1500 = Total_Lumi*kkg_xsecs[20]*Acceptance_m1500;
  ...
  string out_file_loc = "../kkg_files/"+channel+"_cb_"+cur_sys+"_lep_nu_dijet_m.root";
  TFile* CrystalBallFits = new TFile(out_file_loc.c_str(), "RECREATE");
  CrystalBallFits->cd();

  RooDataHist signal_m500("signal_m500","Graviton signal", Mass, h_signal_hist[0]);
  RooDataHist signal_m600("signal_m600","Graviton signal", Mass, h_signal_hist[1]);
  RooDataHist signal_m700("signal_m700","Graviton signal", Mass, h_signal_hist[2]);
  RooDataHist signal_m800("signal_m800","Graviton signal", Mass, h_signal_hist[3]);
  RooDataHist signal_m900("signal_m900","Graviton signal", Mass, h_signal_hist[4]);
  RooDataHist signal_m1000("signal_m1000","Graviton signal", Mass, h_signal_hist[5]);
  // rebin signals empirically above 1000 for better fitting
  h_signal_hist[6]->Rebin(2);
  RooDataHist signal_m1100("signal_m1100","Graviton signal", Mass, h_signal_hist[6]);
  h_signal_hist[7]->Rebin(2);
  RooDataHist signal_m1200("signal_m1200","Graviton signal", Mass, h_signal_hist[7]);
  h_signal_hist[8]->Rebin(2);
  RooDataHist signal_m1300("signal_m1300","Graviton signal", Mass, h_signal_hist[8]);
  h_signal_hist[9]->Rebin(3);
  RooDataHist signal_m1400("signal_m1400","Graviton signal", Mass, h_signal_hist[9]);
  h_signal_hist[10]->Rebin(4);
  RooDataHist signal_m1500("signal_m1500","Graviton signal", Mass, h_signal_hist[10]);
  ...
  CrystalBall3.fitTo(signal_m700,Range(500,850));
  ...
  CrystalBall4.fitTo(signal_m800,Range(600,1000));
  ...
  //signal_m1100.Rebin(2);
  //Hist_m1100->Rebin(2);
  CrystalBall7.fitTo(signal_m1100,Range(700,1300));
  ...
  CrystalBall8.fitTo(signal_m1200,Range(700,1350));
  ...
  //CrystalBall9.fitTo(signal_m1300,Range(1050,1500));
  CrystalBall9.fitTo(signal_m1300,Range(900,1500));
  ...
  CrystalBall10.fitTo(signal_m1400,Range(1000,1650));
  ...
  CrystalBall11.fitTo(signal_m1500,Range(1050,1700));
  ...
  Accept->SetTitle("");
  Accept->GetXaxis()->SetTitle("Graviton Mass [GeV]");
  ...
  string acc_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_signal_acceptances.pdf";
  ...
  TF1* f1_acc_pol_fit = new TF1("f1_acc_pol_fit","pol3(0)",400,800);//[0]+[1]*x+[2]*x^2+[3]*x^3
  TF1* f1_acc_expo_fit = new TF1("f1_acc_expo_fit","expo(0)",795,1600);//
  ...
  Channel_Accept->GetXaxis()->SetTitle("Graviton Mass [GeV]");
  ...
  string chan_acc_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_signal_channel_acceptances.pdf";
  ...
  TF1 * sigma_fit = new TF1("sigma_fit", "[0]*sqrt([1]*(x-[2]))",500,1200);
  ...
  TF1 * mean_fit = new TF1("mean_fit", "[0]+[1]*x",500,1400);
  ...
  //TF1 * a_fit = new TF1("a_fit", "[0]/([1]*x^2)+[2]*x",500,1200);
...
//   a_fit->SetParameter(2,1);
  ...
  //TF1 * a_fit = new TF1("a_fit", "[0]*exp(-1.*[1]*(x-[2]))");
  //TF1 * a_fit = new TF1("a_fit", "[0]*exp(-1.*[1]*x)+[2]");
  TF1 * a_fit = new TF1("a_fit", "[0]/([1]*x^2)+[2]*x",500,1300);
  a_fit->SetParameter(0,1);
  //a_fit->SetParameter(1,-2e-3);
  a_fit->SetParameter(1,1e-6);
  a_fit->SetParameter(2,1e-4);

  TFitResultPtr MEAN = gr_parameter_Mean->Fit("mean_fit", "SR");
  TFitResultPtr SIGMA = gr_parameter_Sigma->Fit("sigma_fit", "SR");
  cout<<"!! "<<endl;
  TFitResultPtr A_res = gr_parameter_A->Fit("a_fit", "SR");
  cout<<"!! "<<endl;
  ...
  Hist_m500->Scale(5000*Channel_Acceptance_m500*2);
  Hist_m550->Scale(5000*Channel_Acceptance_m550*2);
  Hist_m600->Scale(5000*Channel_Acceptance_m600*2);
  Hist_m650->Scale(5000*Channel_Acceptance_m650*2);
  Hist_m700->Scale(5000*Channel_Acceptance_m700*2);
  Hist_m750->Scale(5000*Channel_Acceptance_m750*2);
  Hist_m800->Scale(5000*Channel_Acceptance_m800*2);
  Hist_m850->Scale(5000*Channel_Acceptance_m850*2);
  Hist_m900->Scale(5000*Channel_Acceptance_m900*2);
  Hist_m950->Scale(5000*Channel_Acceptance_m950*2);
  Hist_m1000->Scale(5000*Channel_Acceptance_m1000*2);
  Hist_m1050->Scale(5000*Channel_Acceptance_m1050*2);
  Hist_m1100->Scale(5000*Channel_Acceptance_m1100*2);
  Hist_m1150->Scale(5000*Channel_Acceptance_m1150*2);
  Hist_m1200->Scale(5000*Channel_Acceptance_m1200*2);
  Hist_m1250->Scale(5000*Channel_Acceptance_m1250*2);
  Hist_m1300->Scale(5000*Channel_Acceptance_m1300*2);
  Hist_m1350->Scale(5000*Channel_Acceptance_m1350*2);
  Hist_m1400->Scale(5000*Channel_Acceptance_m1400*2);
  Hist_m1450->Scale(5000*Channel_Acceptance_m1450*2);
  Hist_m1500->Scale(5000*Channel_Acceptance_m1500*2);
  Hist_m1550->Scale(5000*Channel_Acceptance_m1550*2);
  Hist_m1600->Scale(5000*Channel_Acceptance_m1600*2);
  Hist_m1650->Scale(5000*Channel_Acceptance_m1650*2);
  Hist_m1700->Scale(5000*Channel_Acceptance_m1700*2);
  Hist_m1750->Scale(5000*Channel_Acceptance_m1750*2);
  Hist_m1800->Scale(5000*Channel_Acceptance_m1800*2);
  Hist_m1850->Scale(5000*Channel_Acceptance_m1850*2);
  Hist_m1900->Scale(5000*Channel_Acceptance_m1900*2);
  Hist_m1950->Scale(5000*Channel_Acceptance_m1950*2);
  Hist_m2000->Scale(5000*Channel_Acceptance_m2000*2);
  

  //Int_t total_bins = 200;
  Int_t total_bins = n_bins;
  ...
  Hist_m500->Scale(Scale_m500/(5000*Channel_Acceptance_m500*2));
  Hist_m550->Scale(Scale_m550/(5000*Channel_Acceptance_m550*2));
  Hist_m600->Scale(Scale_m600/(5000*Channel_Acceptance_m600*2));
  Hist_m650->Scale(Scale_m650/(5000*Channel_Acceptance_m650*2));
  Hist_m700->Scale(Scale_m700/(5000*Channel_Acceptance_m700*2));
  Hist_m750->Scale(Scale_m750/(5000*Channel_Acceptance_m750*2));
  Hist_m800->Scale(Scale_m800/(5000*Channel_Acceptance_m800*2));
  Hist_m850->Scale(Scale_m850/(5000*Channel_Acceptance_m850*2));
  Hist_m900->Scale(Scale_m900/(5000*Channel_Acceptance_m900*2));
  Hist_m950->Scale(Scale_m950/(5000*Channel_Acceptance_m950*2));
  Hist_m1000->Scale(Scale_m1000/(5000*Channel_Acceptance_m1000*2));
  Hist_m1050->Scale(Scale_m1050/(5000*Channel_Acceptance_m1050*2));
  Hist_m1100->Scale(Scale_m1100/(5000*Channel_Acceptance_m1100*2));
  Hist_m1150->Scale(Scale_m1150/(5000*Channel_Acceptance_m1150*2));
  Hist_m1200->Scale(Scale_m1200/(5000*Channel_Acceptance_m1200*2));
  Hist_m1250->Scale(Scale_m1250/(5000*Channel_Acceptance_m1250*2));
  Hist_m1300->Scale(Scale_m1300/(5000*Channel_Acceptance_m1300*2));
  Hist_m1350->Scale(Scale_m1350/(5000*Channel_Acceptance_m1350*2));
  Hist_m1400->Scale(Scale_m1400/(5000*Channel_Acceptance_m1400*2));
  Hist_m1450->Scale(Scale_m1450/(5000*Channel_Acceptance_m1450*2));
  Hist_m1500->Scale(Scale_m1500/(5000*Channel_Acceptance_m1500*2));
  Hist_m1550->Scale(Scale_m1550/(5000*Channel_Acceptance_m1550*2));
  Hist_m1600->Scale(Scale_m1600/(5000*Channel_Acceptance_m1600*2));
  Hist_m1650->Scale(Scale_m1650/(5000*Channel_Acceptance_m1650*2));
  Hist_m1700->Scale(Scale_m1700/(5000*Channel_Acceptance_m1700*2));
  Hist_m1750->Scale(Scale_m1750/(5000*Channel_Acceptance_m1750*2));
  Hist_m1800->Scale(Scale_m1800/(5000*Channel_Acceptance_m1800*2));
  Hist_m1850->Scale(Scale_m1850/(5000*Channel_Acceptance_m1850*2));
  Hist_m1900->Scale(Scale_m1900/(5000*Channel_Acceptance_m1900*2));
  Hist_m1950->Scale(Scale_m1950/(5000*Channel_Acceptance_m1950*2));
  Hist_m2000->Scale(Scale_m2000/(5000*Channel_Acceptance_m2000*2));
  ...
  CrystalBall9.plotOn(m1300frame, LineColor(kBlue),LineStyle(2),Name("cb"),Range(1000,1500));
  ...
  lgnd_m500->AddEntry("cb","CB Fit","L");
  lgnd_m500->Draw();
  
  string tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m500_fit_templates.pdf";
  ...
  lgnd_m600->AddEntry("cb","CB Fit","L");
  lgnd_m600->Draw();
  
  tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m600_fit_templates.pdf";
  ...
  lgnd_m700->AddEntry("cb","CB Fit","L");
  lgnd_m700->Draw();
  
  tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m700_fit_templates.pdf";
  ...
  lgnd_m800->AddEntry("cb","CB Fit","L");
  lgnd_m800->Draw();
  
  tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m800_fit_templates.pdf";
  ...
  lgnd_m900->AddEntry("cb","CB Fit","L");
  lgnd_m900->Draw();
  
  tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m900_fit_templates.pdf";
  ...
  lgnd_m1000->AddEntry("cb","CB Fit","L");
  lgnd_m1000->Draw();
  
  tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m1000_fit_templates.pdf";
  ...
  lgnd_m1100->AddEntry("cb","CB Fit","L");
  lgnd_m1100->Draw();
  
  tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m1100_fit_templates.pdf";
  ...
  lgnd_m1200->AddEntry("cb","CB Fit","L");
  lgnd_m1200->Draw();
  
  tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m1200_fit_templates.pdf";
  ...
  lgnd_m1300->AddEntry("cb","CB Fit","L");
  lgnd_m1300->Draw();
  
  tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m1300_fit_templates.pdf";
  ...
  lgnd_m1400->AddEntry("cb","CB Fit","L");
  lgnd_m1400->Draw();
  
  tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m1400_fit_templates.pdf";
  ...
  lgnd_m1500->AddEntry("cb","CB Fit","L");
  lgnd_m1500->Draw();

  tmp_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_m1500_fit_templates.pdf";
  ...
  gr_parameter_Mean->GetXaxis()->SetTitle("Graviton Mass [GeV]");
  c_parm->cd(2);
  ...
  gr_parameter_Sigma->GetXaxis()->SetTitle("Graviton Mass [GeV]");
  c_parm->cd(3);
  ...
  gr_parameter_A->GetXaxis()->SetTitle("Graviton Mass [GeV]");
  ...
  gr_parameter_N->GetXaxis()->SetTitle("Graviton Mass [GeV]");

  string cur_saveas = "../kkg_plots/"+channel+"_"+cur_sys+"_fit_parameters.pdf";
...
void signalShapesKKG::makeClosurePlots(string channel/*="evjj"*/,string cur_sys/*="jes_nom"*/){
  ...
  string cb_file_name = "../kkg_files/"+channel+"_cb_"+cur_sys+"_lep_nu_dijet_m.root";
  TFile* cur_cb_file = TFile::Open(cb_file_name.c_str(),"READ");

  TH1F* cb_hists[NGmass];
  char cb_name[200];
  string cb_string = "cb_kkg_m%i";
  ...
  TH1F* h_signals[n_sig_masses];

  //m500
  string signal_m500_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m500.systematics.plot.root";
  ...
  h_signals[0] = (TH1F*) signal_m500_hist->Clone();
  
  
  //m600
  string signal_m600_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m600.systematics.plot.root";
  ...
  h_signals[1] = (TH1F*) signal_m600_hist->Clone();



  //m700
  string signal_m700_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m700.systematics.plot.root";
  ...
  h_signals[2] = (TH1F*) signal_m700_hist->Clone();

  //m800
  string signal_m800_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m800.systematics.plot.root";
  ...
  h_signals[3] = (TH1F*) signal_m800_hist->Clone();

  //m900
  string signal_m900_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m900.systematics.plot.root";
  ...
  h_signals[4] = (TH1F*) signal_m900_hist->Clone();

  //m1000
  string signal_m1000_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m1000.systematics.plot.root";
  ...
  h_signals[5] = (TH1F*) signal_m1000_hist->Clone();


  //m1100
  string signal_m1100_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m1100.systematics.plot.root";
  ...
  h_signals[6] = (TH1F*) signal_m1100_hist->Clone();


  //m1200
  string signal_m1200_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m1200.systematics.plot.root";
  ...
  h_signals[7] = (TH1F*) signal_m1200_hist->Clone();

  //m1300
  string signal_m1300_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m1300.systematics.plot.root";
  ...
  h_signals[8] = (TH1F*) signal_m1300_hist->Clone();

  //m1400
  string signal_m1400_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m1400.systematics.plot.root";
  ...
  h_signals[9] = (TH1F*) signal_m1400_hist->Clone();

  //m1500
  string signal_m1500_file_name =
    "../../plots/systematics_plots/merged/mc.afii.kkg.lvjj.m1500.systematics.plot.root";
  ...
  h_signals[10] = (TH1F*) signal_m1500_hist->Clone();
  ...
  TLegend *lgnd_m500 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m500->SetFillColor(kWhite);
  lgnd_m500->SetLineColor(kWhite);
  lgnd_m500->AddEntry(signal_m500_hist,"Signal", "P");
  lgnd_m500->AddEntry(cb_hists[0],"CB Template","P");
  lgnd_m500->Draw();

  string tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m500_closure.pdf";
  ...
  TLegend *lgnd_m600 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m600->SetFillColor(kWhite);
  lgnd_m600->SetLineColor(kWhite);
  lgnd_m600->AddEntry(signal_m600_hist,"Signal", "P");
  lgnd_m600->AddEntry(cb_hists[2],"CB Template","P");
  lgnd_m600->Draw();

  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m600_closure.pdf";
  ...
  signal_m700_hist->GetXaxis()->SetRangeUser(300,1000);
  ...
  TLegend *lgnd_m700 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m700->SetFillColor(kWhite);
  lgnd_m700->SetLineColor(kWhite);
  lgnd_m700->AddEntry(signal_m700_hist,"Signal", "P");
  lgnd_m700->AddEntry(cb_hists[4],"CB Template","P");
  lgnd_m700->Draw();

  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m700_closure.pdf";
  ...
  signal_m800_hist->GetXaxis()->SetRangeUser(300,1200);
  ...
  TLegend *lgnd_m800 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m800->SetFillColor(kWhite);
  lgnd_m800->SetLineColor(kWhite);
  lgnd_m800->AddEntry(signal_m800_hist,"Signal", "P");
  lgnd_m800->AddEntry(cb_hists[6],"CB Template","P");
  lgnd_m800->Draw();

  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m800_closure.pdf";
  ...
  TLegend *lgnd_m900 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m900->SetFillColor(kWhite);
  lgnd_m900->SetLineColor(kWhite);
  lgnd_m900->AddEntry(signal_m900_hist,"Signal", "P");
  lgnd_m900->AddEntry(cb_hists[8],"CB Template","P");
  lgnd_m900->Draw();

  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m900_closure.pdf";
  ...
  signal_m1000_hist->GetXaxis()->SetRangeUser(400,1400);
  ...
  TLegend *lgnd_m1000 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m1000->SetFillColor(kWhite);
  lgnd_m1000->SetLineColor(kWhite);
  lgnd_m1000->AddEntry(signal_m1000_hist,"Signal", "P");
  lgnd_m1000->AddEntry(cb_hists[10],"CB Template","P");
  lgnd_m1000->Draw();

  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m1000_closure.pdf";
  ...
  signal_m1100_hist->GetXaxis()->SetRangeUser(400,1400);
  ...
  TLegend *lgnd_m1100 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m1100->SetFillColor(kWhite);
  lgnd_m1100->SetLineColor(kWhite);
  lgnd_m1100->AddEntry(signal_m1100_hist,"Signal", "P");
  lgnd_m1100->AddEntry(cb_hists[12],"CB Template","P");
  lgnd_m1100->Draw();

  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m1100_closure.pdf";
  ...
  signal_m1200_hist->GetXaxis()->SetRangeUser(400,1600);
  ...
  TLegend *lgnd_m1200 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m1200->SetFillColor(kWhite);
  lgnd_m1200->SetLineColor(kWhite);
  lgnd_m1200->AddEntry(signal_m1200_hist,"Signal", "P");
  lgnd_m1200->AddEntry(cb_hists[14],"CB Template","P");
  lgnd_m1200->Draw();

  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m1200_closure.pdf";
  ...
  signal_m1300_hist->GetXaxis()->SetRangeUser(500,1600);
  ...
  TLegend *lgnd_m1300 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m1300->SetFillColor(kWhite);
  lgnd_m1300->SetLineColor(kWhite);
  lgnd_m1300->AddEntry(signal_m1300_hist,"Signal", "P");
  lgnd_m1300->AddEntry(cb_hists[16],"CB Template","P");
  lgnd_m1300->Draw();

  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m1300_closure.pdf";
  ...
  signal_m1400_hist->GetXaxis()->SetRangeUser(500,1800);
  ...
  TLegend *lgnd_m1400 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m1400->SetFillColor(kWhite);
  lgnd_m1400->SetLineColor(kWhite);
  lgnd_m1400->AddEntry(signal_m1400_hist,"Signal", "P");
  lgnd_m1400->AddEntry(cb_hists[18],"CB Template","P");
  lgnd_m1400->Draw();

  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m1400_closure.pdf";
  ...
  signal_m1500_hist->GetXaxis()->SetRangeUser(600,1800);
  ...
  TLegend *lgnd_m1500 = new TLegend(0.2,0.73,0.45,0.87);
  lgnd_m1500->SetFillColor(kWhite);
  lgnd_m1500->SetLineColor(kWhite);
  lgnd_m1500->AddEntry(signal_m1500_hist,"Signal", "P");
  lgnd_m1500->AddEntry(cb_hists[20],"CB Template","P");
  lgnd_m1500->Draw();

  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_m1500_closure.pdf";
  ...
  tmp_saveas = "../kkg_plots/template_plots/"+channel+"_"+cur_sys+"_template_plots.pdf";
  c_template_plots->SaveAs(tmp_saveas.c_str());

  //////////////////////////////////////////////////
  // AFII signal masses
  ////////////////////////////////////////////////// 
  string tc_kkg_signal_m_name = "tc_"+channel+"_"+cur_sys+"_kkg_signal_m";
  TCanvas * tc_kkg_signal_m =
    new TCanvas(tc_kkg_signal_m_name.c_str(),"Graviton M(lvjj)",200,10,700,500);
  tc_kkg_signal_m->cd();

  
  TLegend *lgnd_kkg_signal_m = new TLegend(0.75,0.45,0.9,0.92);
  lgnd_kkg_signal_m->SetFillColor(kWhite);
  lgnd_kkg_signal_m->SetLineColor(kWhite);
  lgnd_kkg_signal_m->SetBorderSize(0);
  lgnd_kkg_signal_m->SetTextSize(0.035);
  lgnd_kkg_signal_m->SetHeader("Signal Mass [GeV]");

  char sig_mass[200];      
  for(int j=0;j<n_sig_masses;j++){
    string draw_opt = (j==0) ? "hist" : "hist same";

    sprintf(sig_mass,"%i",signal_masses_int[j]);

    if(j==0){
      ((TH1F*) h_signals[j])->GetXaxis()->SetRangeUser(0,2000);
      ((TH1F*) h_signals[j])->GetXaxis()->SetTitle("M(l#nu jj) [GeV]");
      ((TH1F*) h_signals[j])->GetYaxis()->SetRangeUser(0.0001,200);
    }

    ((TH1F*) h_signals[j])->SetFillColor(kAzure+2*j);
    ((TH1F*) h_signals[j])->Draw(draw_opt.c_str());

    lgnd_kkg_signal_m->AddEntry(h_signals[j],sig_mass,"F");

  }

  lgnd_kkg_signal_m->Draw();  
  tc_kkg_signal_m->SetLogy();
  //tc_kkg_signal_m->SetLogx();
  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_signal_m.pdf";
  tc_kkg_signal_m->SaveAs(tmp_saveas.c_str());

  //////////////////////////////////////////////////
  // CB templates
  ////////////////////////////////////////////////// 

  string tc_kkg_cb_m_name = "tc_"+channel+"_"+cur_sys+"_kkg_cb_m";
  TCanvas * tc_kkg_cb_m =
    new TCanvas(tc_kkg_cb_m_name.c_str(),"Graviton M(lvjj)",200,10,700,500);
  tc_kkg_cb_m->cd();

  
  TLegend *lgnd_kkg_cb_m = new TLegend(0.8,0.35,0.95,0.92);
  lgnd_kkg_cb_m->SetFillColor(kWhite);
  lgnd_kkg_cb_m->SetLineColor(kWhite);
  lgnd_kkg_cb_m->SetBorderSize(0);
  lgnd_kkg_cb_m->SetTextSize(0.035);
  lgnd_kkg_cb_m->SetHeader("Mass [GeV]");

  char cb_mass[200];
  char signal_mass[200];      
  for(int j=0;j<n_sig_masses;j++){
    string draw_opt = (j==0) ? "hist" : "hist same";

    sprintf(cb_mass,"%i",GmassInt[2*j+1]);
    sprintf(signal_mass,"%i",signal_masses_int[j]);

    ((TH1F*) h_signals[j])->Rebin(4);
    if(j==0){
      // ((TH1F*) cb_hists[2*j+1])->GetXaxis()->SetRangeUser(0,2000);
      // ((TH1F*) cb_hists[2*j+1])->GetXaxis()->SetTitle("M(l#nu jj) [GeV]");
      // ((TH1F*) cb_hists[2*j+1])->GetYaxis()->SetRangeUser(0.0001,500);
      
      ((TH1F*) h_signals[j])->GetXaxis()->SetRangeUser(0,2000);
      ((TH1F*) h_signals[j])->GetXaxis()->SetTitle("M(l#nu jj) [GeV]");
      ((TH1F*) h_signals[j])->GetYaxis()->SetRangeUser(0.0001,500);
    }
    
    ((TH1F*) h_signals[j])->SetFillColor(kAzure+2*j);
    //((TH1F*) h_signals[j])->SetLineColor(kAzure+2*j);
    ((TH1F*) h_signals[j])->Draw(draw_opt.c_str());

    ((TH1F*) cb_hists[2*j+1])->SetFillColor(kAzure+(2*j+1));
    //((TH1F*) cb_hists[2*j+1])->SetLineColor(kAzure+(2*j+1));
    ((TH1F*) cb_hists[2*j+1])->Draw("hist same");

    lgnd_kkg_cb_m->AddEntry(h_signals[j],signal_mass,"F");
    lgnd_kkg_cb_m->AddEntry(cb_hists[2*j+1],cb_mass,"F");
  ...
  tc_kkg_cb_m->SetLogy();
  //tc_kkg_cb_m->SetLogx();
  tmp_saveas = "../kkg_plots/closure_plots/"+channel+"_"+cur_sys+"_cb_m.pdf";
  tc_kkg_cb_m->SaveAs(tmp_saveas.c_str());
  ...
  for(int k=0;k<n_sig_masses;k++)
    if(h_signals[k]) delete (TH1F*) h_signals[k];

  //if(h_sum_signals) delete h_sum_signals;
