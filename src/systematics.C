...
Bool_t systematics::Process(Long64_t entry)
{
  ...
  for(int m=0;m<nMods;m++){
    ...
    if(sys_lep_pt>lep_pt_cut &&  //lep_pt>30
       sys_jes_lead_jet_tlv[m].Pt()>lead_jet_pt_cut && // lead jet pt > 100
       sys_jes_second_jet_tlv[m].Pt()>jet_pt_cut && // second jet pt > 40
       sys_jes_met_et[m]>met_cut){ // met > 40
...
bool systematics::PassSigWW(float cur_lep_met_pt,TLorentzVector cur_tlv_lead_jet,TLorentzVector cur_tlv_second_jet){
  ...
  return false;// if pass preselection
  //
