...
//////////////////////////////////////////////////
// Loaded in lvjj_helper.h
//////////////////////////////////////////////////
// Constants
// bool doPartialUnblind=true;// unblind 1fb-1
//
// Functions
// passElecTri(nElectrons,lep_met_dphi,rf_met_et);
...
TH1F* h_lep_eta_frths[nCtrls][nProcs];
...
void skimmedGWW::Begin(TTree * /*tree*/)
{
   ...
   for(int c=0;c<nCtrls;c++){//control regions
	 ...
	 new TH2F(lep_met_dphi_met_name.c_str(),lep_met_dphi_met_title.c_str(),
       ...
       //h_lep_pt[c][i] =	 new TH1F(lep_pt_name.c_str(),lep_pt_title.c_str(),1000,0,1000);
       ...
       string lep_eta_frths_name = h_proc+"_lep_eta_frths";
       string lep_eta_frths_title = lep_eta_frths_name;
       h_lep_eta_frths[c][i] = new TH1F(lep_eta_frths_name.c_str(),lep_eta_frths_title.c_str(),100,-3,4);
       ...
       string lead_jet_eta_frths_name = h_proc+"_lead_jet_eta_frths";
       string lead_jet_eta_frths_title = lead_jet_eta_frths_name;
       h_lead_jet_eta_frths[c][i] = new TH1F(lead_jet_eta_frths_name.c_str(),lead_jet_eta_frths_title.c_str(),50,-3,3.5);
	 ...
	 //new TH1F(lead_jet_pt_name.c_str(),lead_jet_pt_title.c_str(),1000,0,1000);
	 ...
	 new TH1F(second_jet_pt_name.c_str(),second_jet_pt_title.c_str(),second_jet_pt_bin_n,0,second_jet_pt_max_x);
       ...
       //h_met[c][i] = new TH1F(met_name.c_str(),met_title.c_str(),1000,0,1000);
	 ...
	 new TH1F(lep_nu_dr_name.c_str(),lep_nu_dr_title.c_str(),30,0,2.5*TMath::Pi());
       ...
       string lep_nu_e_diff_name = h_proc+"_lep_nu_e_diff";
       string lep_nu_e_diff_title = lep_nu_e_diff_name;
       h_lep_nu_e_diff[c][i] = new TH1F(lep_nu_e_diff_name.c_str(),lep_nu_e_diff_title.c_str(),30,0,1);
       ...
       string lep_d0_name = h_proc+"_lep_d0";
       string lep_d0_title = lep_d0_name;
       h_lep_d0[c][i] = new TH1F(lep_d0_name.c_str(),lep_d0_title.c_str(),45,-0.3,0.3);
       ...
       h_lt[c][i] = new TH1F(lt_name.c_str(),lt_title.c_str(),25,0,lt_max_x);
	 ...
	 new TH1F(rwt_lep_met_pt_name.c_str(),rwt_lep_met_pt_title.c_str(),rwt_lep_met_pt_bin_n,0,rwt_lep_met_pt_max_x);
       ...
       int lep_met_mt_bin_n = (isEWK) ? 25 : 45;
	 ...
	 new TH1F(lep_met_mt_name.c_str(),lep_met_mt_title.c_str(),lep_met_mt_bin_n,lep_met_mt_min_x,lep_met_mt_max_x);
       ...
       int lep_met_mt_eq2j_bin_n = (isEWK) ? 25 : 45;
	 ...
	 new TH1F(lep_met_mt_eq2j_name.c_str(),lep_met_mt_eq2j_title.c_str(),lep_met_mt_eq2j_bin_n,lep_met_mt_eq2j_min_x,lep_met_mt_eq2j_max_x);
       ...
       int lep_met_mt_gt2j_bin_n = (isEWK) ? 25 : 45;
	 ...
	 new TH1F(sig_lep_nu_dijet_m_name.c_str(),sig_lep_nu_dijet_m_title.c_str(),28,0,sig_lep_nu_dijet_m_max_x);
       ...
       h_ht[c][i] = new TH1F(ht_name.c_str(),ht_title.c_str(),50,0,1100);
...
Bool_t skimmedGWW::Process(Long64_t entry)
{
  ...
  if(lvjjHelper::doPartialUnblind && is_data){
    float cur_rand = m_rand->Rndm();
    //if(entry<20) cout<<cur_rand<<endl;
    if(cur_rand>(1./4.70135)) return kTRUE;

  }


  // double lep_met_dphi = rf_met_phi-lvjj_lep_phi;
  ...
  // // lower triangle cut (both channels)
  // bool pass_tri = lvjjHelper::passTri(lep_met_dphi,rf_met_et);

  // // upper triangle cut (only evjj)
  // bool pass_elec_tri = lvjjHelper::passElecTri(nGoodElectrons,lep_met_dphi,rf_met_et);

  // upper/lower triangle cuts for electrons, only lower for muons
  ...
  // moved d0sig cut offline for systematics (incomplete)
  // cut made at skim level for QCD 
  bool passMuonSigD0 = (nGoodMuons && !qcd_event) ? fabs(lvjj_lep_d0sig)<3. : true;

  // wjets calo iso cut is loosened
  // cut made at skim level for QCD 
  bool passElecCaloIso = (nGoodElectrons && !qcd_event) ? lvjj_lep_calo_iso<6 : true;

  // TMP, had removed cut on qcd calo_iso to improve QCD statistics
  // checking distributions with it back in
  ...
  //ctrl_all: lep_pt>30, lead_jet_pt>40, second_jet_pt>40
  bool passBaseLine = (passMuonSigD0 &&
		       ctrl_all &&
		       rf_met_et>tmp_met_cut &&
		       lead_jet_pt>100. && // tested 2012/04/23
		       passMuonQCDCaloIso &&
		       passElecCaloIso && 
		       dijet_dr<4.0);
		       
  // used for qcd estimation
  bool passLoose =  passBaseLine;
  
  bool passPS = (passLoose &&
		 pass_tri &&
  		 fabs(lvjj_lep_sig_diff_qoverp)<5.);
  
  bool passOption = (passLoose && fabs(lvjj_lep_sig_diff_qoverp)<5.);
  ...
  // double lead_jet_met_dphi = rf_met_phi-lead_jet_phi;
  ...
  float lead_jet_met_dphi = lvjjHelper::getDeltaPhi(rf_met_phi,lead_jet_phi);
  ...
  float dijet_dphi = lvjjHelper::getDeltaPhi(lead_jet_phi,second_jet_phi);
  ...
  float alpgen_rwt = (lvjjHelper::doAlpgenReweighting && cur_option.find("alpgen.w")!=string::npos) ? evt_pu_weight*evt_alpgen_weight : evt_pu_weight;
  sow+=alpgen_rwt;
  ...
  for(int ctrlIndex=0;ctrlIndex<nCtrls;ctrlIndex++){
    ...
    if(ctrlIndex==1) sow_ps+=alpgen_rwt;
    ...
    scat_lep_met_dphi_met[ctrlIndex][procIndex]->Fill(rf_met_et,lep_met_dphi);
    ...
    //MET Plots
    h_set[ctrlIndex][procIndex]->Fill(rf_met_set,alpgen_rwt);
    h_met[ctrlIndex][procIndex]->Fill(rf_met_et,alpgen_rwt);
    h_met_phi[ctrlIndex][procIndex]->Fill(rf_met_phi,alpgen_rwt);
    
    float lep_nu_e_diff = fabs(tlv_lep.E()-tlv_nu.E())/(tlv_lep.E()+tlv_nu.E());

    h_lep_nu_e_diff[ctrlIndex][procIndex]->Fill(lep_nu_e_diff,alpgen_rwt);
    if(nGoodJets==2) h_met_2j[0][procIndex]->Fill(rf_met_et,alpgen_rwt);
    if(nGoodJets==3) h_met_3j[0][procIndex]->Fill(rf_met_et,alpgen_rwt);
    if(nGoodJets>=4) h_met_4j[0][procIndex]->Fill(rf_met_et,alpgen_rwt);
    ...
    h_lead_jet_eta_frths[ctrlIndex][procIndex]->Fill(lead_jet_eta,alpgen_rwt);
    ...
    //h_lep_d0[ctrlIndex][procIndex]->Fill(lvjj_lep_d0,alpgen_rwt);
    h_lep_absz0[ctrlIndex][procIndex]->Fill(lvjj_lep_absz0,alpgen_rwt);
    h_lep_eta[ctrlIndex][procIndex]->Fill(lvjj_lep_eta,alpgen_rwt);
    h_lep_eta_frths[ctrlIndex][procIndex]->Fill(lvjj_lep_eta,alpgen_rwt);
...
void skimmedGWW::Terminate()
{
  ...
  // the results graphically or save the results to file.
   ...
   cout<<"PS Sum of Weights: "<<sow_ps<<endl;
   cout<<endl;
  ...
  for(int i=0;i<nCtrls;i++){
    ...
    for(int j=0;j<nProcs;j++){
      ...
      delete (TH1F*) h_lep_nu_e_diff[i][j];
      ...
      delete (TH1F*) h_lead_jet_eta_frths[i][j];
