...
TH1F* h_mex[nProcs];
TH1F* h_mey[nProcs];
TH1F* h_adjusted_met[nProcs];
TH1F* h_adjusted_mex[nProcs];
TH1F* h_adjusted_mey[nProcs];
...
void metResolution::Begin(TTree * /*tree*/)
{
  ...
  plotFile->cd();
   ...
   for(int p=0;p<nProcs;p++){
     ...
     hist_name=cur_name+"_adjusted_mex";
     h_adjusted_mex[p]=new TH1F(hist_name.c_str(),hist_name.c_str(),100,-500,500);

     hist_name=cur_name+"_adjusted_mey";
     h_adjusted_mey[p]=new TH1F(hist_name.c_str(),hist_name.c_str(),100,-500,500);

     hist_name=cur_name+"_adjusted_met";
     h_adjusted_met[p]=new TH1F(hist_name.c_str(),hist_name.c_str(),100,0,500);
...
Bool_t metResolution::Process(Long64_t entry)
{
  ...
  float adjusted_mey = (ww_met_ety<0) ? ww_met_ety+sqrt(15) : ww_met_ety-sqrt(15);
  h_adjusted_mey[procIndex]->Fill(adjusted_mey);
  float adjusted_mex = (ww_met_etx<0) ? ww_met_etx+sqrt(15) : ww_met_etx-sqrt(15);
  h_adjusted_mex[procIndex]->Fill(adjusted_mex);
  float adjusted_met = sqrt((adjusted_mex*adjusted_mex)+(adjusted_mey*adjusted_mey));
  h_adjusted_met[procIndex]->Fill(adjusted_met);
