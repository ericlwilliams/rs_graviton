#define wjetsTemplates_cxx
#include "wjetsTemplates.h"
...
void wjetsTemplates::makeSignalParameterFits(string channel/*="evjj"*/,string cur_sys/*="jes_nom"*/){
  ...
  string signal_file_name = file_loc+"mc.alpgen.wjets.systematics.plot.root";
  TFile* cur_file = TFile::Open(signal_file_name.c_str());
  ...
  double cross_section_wjets = 1.196*(26248.7/3);// kfac*xsec(tot/3)
  
  cur_hist->SetDirectory(gDirectory);
  
  // --- Observable ---
  RooRealVar Mass("Mass","Mass(lvjj) (GeV)",0,1600);
  //RooPlot * Fitframe = Mass.frame(); 
  RooPlot * wjets_frame = Mass.frame(0,1600);
  //here

  RooRealVar cb_mean1("cb_mean1","Mean of the Gaussian half of the Crystal Ball", 500, 400, 600);
  ...
  RooRealVar //define the data variables and fit model parameters
    rmass("rmass"  ,"Resonance Mass"    , 500  ,400, 600),
    width("width"  ,"Resonance Width"   , 50 ,0, 100),
    bgshape("bgshape","Background shape"  ,0 ,-10.0, 0.0),
    frac("frac"   ,"Signal fraction"   , 0.5  ,0.0, 1.0) ;
  // Create the fit model components: Gaussian and exponential PDFs
  RooGaussian bg_gauss("bg_gauss","Background Gauss Distribution",Mass,rmass,width);
  RooExponential bg_expo("bg_expo","Background distribution",Mass,bgshape) ;
  // Combine them using addition (with relative fraction parameter)
  //RooAddPdf model("model","Signal + Background",signal,bg,frac) ;

  string out_file_loc = "../files/"+channel+"_wjets_"+cur_sys+"_lep_nu_dijet_m.root";
  TFile* WjetsFits = new TFile(out_file_loc.c_str(), "RECREATE");
  WjetsFits->cd();

  RooDataHist signal_wjets("signal_wjets","Wjets signal", Mass, cur_hist);

  string hist_name = channel+"_"+cur_sys+"_";
  TH1 * Hist_wjets = Mass.createHistogram("Signal wjets", Binning(n_bins));
  string hist_wjets_name = "wjets_bgshape";
  Hist_wjets->SetName(hist_wjets_name.c_str());

  //RooFitResult* expo_fit_result =  bg_expo.fitTo(signal_wjets,Range(560,1200),Save());
  //bg_expo.fitTo(signal_wjets,Range(560,1200));

  //RooRealVar MASS("Mass","Mass(lvjj) (GeV)", 560., 1200.);
  bg_expo.fitTo(signal_wjets,Range(560,1200),Save());
  bg_expo.fillHistogram(Hist_wjets,RooArgList(Mass));
  
  for(int i=1;i<=Hist_wjets->GetNbinsX();i++){
    if(Hist_wjets->GetBinCenter(i)<560 || 
       Hist_wjets->GetBinCenter(i)>1200){
      Hist_wjets->SetBinContent(i,0.);
      Hist_wjets->SetBinError(i,0.);
    }
  }
  Hist_wjets->Scale(1./Hist_wjets->Integral());
  
  TH1F* norm_hist = (TH1F*) cur_hist->Clone();
  //norm_hist->SetDirectory(0);
  norm_hist->Scale(1./norm_hist->Integral());
    
  //////////////////////////////////////////////////
  // Take bins up to 560 from background
  ////////////////////////////////////////////////// 
  
  for(int j=1;j<=norm_hist->GetNbinsX();j++){
    if(norm_hist->GetBinCenter(j)>560) break;
    Double_t cur_bin_val = norm_hist->GetBinContent(j);
    Double_t cur_bin_err = norm_hist->GetBinError(j);


    Hist_wjets->SetBinContent(j,cur_bin_val);
    Hist_wjets->SetBinError(j,cur_bin_err);
  }
  ...
  // expo_fit_result->Print("v");

  // const TMatrixDSym& cov = expo_fit_result->covarianceMatrix() ;
  ...
  signal_wjets.plotOn(wjets_frame,LineColor(1),MarkerColor(1),MarkerSize(1.0),Name("data"));
  bg_expo.plotOn(wjets_frame, LineColor(kBlue),LineStyle(2),Name("bg"));

  cout<<endl;
  cout<<"chi2: "<<wjets_frame->chiSquare(1)<<endl;
  cout<<endl;

  string tc_wjets_fit_templates_name = channel+"_"+cur_sys+"_wjets_fit_templates";  
  TCanvas* c_wjets_fit_templates =
    new TCanvas(tc_wjets_fit_templates_name.c_str(),"Template distributions",200,10,700,500);
  c_wjets_fit_templates->cd();
  wjets_frame->GetYaxis()->SetRangeUser(0.1,100);
  wjets_frame->Draw();

  c_wjets_fit_templates->SetLogy();
  TLegend *lgnd_wjets = new TLegend(0.65,0.73,0.86,0.87);
  lgnd_wjets->SetFillColor(kWhite);
  lgnd_wjets->SetLineColor(kWhite);
  lgnd_wjets->AddEntry("data","Wjets MC", "P");
  lgnd_wjets->AddEntry("bg","Wjets BG Model","L");
  lgnd_wjets->Draw();
  
  string tmp_saveas = "../plots/"+channel+"_"+cur_sys+"_wjets_fit_templates.pdf";
  c_wjets_fit_templates->SaveAs(tmp_saveas.c_str());
...
//   string out_file_loc = "../rsg_files/"+channel+"_cb_"+cur_sys+"_lep_nu_dijet_m.root";
//   TFile* CrystalBallFits = new TFile(out_file_loc.c_str(), "RECREATE");
//   CrystalBallFits->cd();

//   RooDataHist signal_m500("signal_m500","G* signal", Mass, h_signal_hist[0]);
//   RooDataHist signal_m750("signal_m750","G* signal", Mass, h_signal_hist[1]);
//   RooDataHist signal_m1000("signal_m1000","G* signal", Mass, h_signal_hist[2]);
//   RooDataHist signal_m1250("signal_m1250","G* signal", Mass, h_signal_hist[3]);
//   RooDataHist signal_m1500("signal_m1500","G* signal", Mass, h_signal_hist[4]);
...
//   Accept->SetTitle("");
//   Accept->GetXaxis()->SetTitle("G* Mass [GeV]");
...
//   string acc_saveas = "../rsg_plots/"+channel+"_"+cur_sys+"_signal_acceptances.pdf";
...
//   Channel_Accept->GetXaxis()->SetTitle("G* Mass [GeV]");
...
//   string chan_acc_saveas = "../rsg_plots/"+channel+"_"+cur_sys+"_signal_channel_acceptances.pdf";
...
//   Double_t mass;
//   const int n_template_masses=21;
...
//   //string hist_m1150_name = hist_name+"m1150_cb_lep_nu_dijet_m";
...
//   //string hist_m1200_name = hist_name+"m1200_cb_lep_nu_dijet_m";
...
//   //string hist_m1300_name = hist_name+"m1300_cb_lep_nu_dijet_m";
...
//   //string hist_m1350_name = hist_name+"m1350_cb_lep_nu_dijet_m";
...
//   //string hist_m1400_name = hist_name+"m1400_cb_lep_nu_dijet_m";
...
//   //string hist_m1450_name = hist_name+"m1450_cb_lep_nu_dijet_m";
...
//   // // //CrystalBall_m1250.plotOn(m1250frame,LineColor(4),LineWidth(3));
...
//   string tmp_saveas = "../rsg_plots/"+channel+"_"+cur_sys+"_m500_fit_templates.pdf";
...
//   tmp_saveas = "../rsg_plots/"+channel+"_"+cur_sys+"_m750_fit_templates.pdf";
...
//   tmp_saveas = "../rsg_plots/"+channel+"_"+cur_sys+"_m1000_fit_templates.pdf";
...
//   tmp_saveas = "../rsg_plots/"+channel+"_"+cur_sys+"_m1250_fit_templates.pdf";
...
//   tmp_saveas = "../rsg_plots/"+channel+"_"+cur_sys+"_m1500_fit_templates.pdf";
...
//   // string tmp_saveas = "../rsg_plots/"+channel+"_"+cur_sys+"_signal_templates.pdf";
...
//   gr_parameter_Mean->GetXaxis()->SetTitle("G* Mass [GeV]");
//   c_parm->cd(2);
...
//   gr_parameter_Sigma->GetXaxis()->SetTitle("G* Mass [GeV]");
//   c_parm->cd(3);
...
//   gr_parameter_A->GetXaxis()->SetTitle("G* Mass [GeV]");
...
//   gr_parameter_N->GetXaxis()->SetTitle("G* Mass [GeV]");

//   string cur_saveas = "../rsg_plots/"+channel+"_"+cur_sys+"_fit_parameters.pdf";
...
//   // TGraph * Accept = new TGraph(n_sig_masses,x,Y);
//   // Accept->SetTitle("");
//   // Accept->GetXaxis()->SetTitle("G* Pole Mass [GeV]");
//   // Accept->GetYaxis()->SetTitle("Total Acceptance");
//   // Accept->Draw("AL*");
//   // Accept->SetLineColor(4);
//   // Accept->SetLineWidth(2);
...
//   // cur_saveas = "../rsg_plots/"+channel+"_"+cur_sys+"_signal_acceptances.pdf";
...
//   if(lgnd_m1500) delete lgnd_m1500;
 ...
 void wjetsTemplates::makeClosurePlots(string channel/*="evjj"*/,string cur_sys/*="jes_nom"*/){
...
//   string cb_file_name = "../rsg_files/"+channel+"_cb_"+cur_sys+"_lep_nu_dijet_m.root";
...
//     "../../plots/systematics_plots/merged/mc.rsg.m500.kmpl0_1.systematics.plot.root";
...
//     "../../plots/systematics_plots/merged/mc.rsg.m750.kmpl0_1.systematics.plot.root";
...
//     "../../plots/systematics_plots/merged/mc.rsg.m1000.kmpl0_1.systematics.plot.root";
...
//     "../../plots/systematics_plots/merged/mc.rsg.m1250.kmpl0_1.systematics.plot.root";
...
//     "../../plots/systematics_plots/merged/mc.rsg.m1500.kmpl0_1.systematics.plot.root";
...
//   string tmp_saveas = "../rsg_plots/closure_plots/"+channel+"_"+cur_sys+"_m500_closure.pdf";
...
//   signal_m750_hist->Rebin(rebin_n/4);
...
//   tmp_saveas = "../rsg_plots/closure_plots/"+channel+"_"+cur_sys+"_m750_closure.pdf";
...
//   tmp_saveas = "../rsg_plots/closure_plots/"+channel+"_"+cur_sys+"_m1000_closure.pdf";
...
//   signal_m1250_hist->Rebin(rebin_n/4);
...
//   tmp_saveas = "../rsg_plots/closure_plots/"+channel+"_"+cur_sys+"_m1250_closure.pdf";
...
//   tmp_saveas = "../rsg_plots/closure_plots/"+channel+"_"+cur_sys+"_m1500_closure.pdf";
...
//   tmp_saveas = "../rsg_plots/template_plots/"+channel+"_"+cur_sys+"_template_plots.pdf";
