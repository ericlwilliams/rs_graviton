...
void runTest(string sys="jes_nom"){

  string cur_sys = sys;

  gROOT->ProcessLine(".L ../src/wjetsTemplates.C+");
  wjetsTemplates* sigShape = new wjetsTemplates();

  string test_channel = "evjj";
  string test_run = "jes_nom";
  sigShape->makeSignalParameterFits(test_channel.c_str(),test_run.c_str());
