...
void wjetsJES(){
  ...
  double muon_lowsb_nom_count = h_muon_jes_nom_dijet_m->IntegralAndError(1,lowsb_bin,muon_lowsb_nom_err);

  double muon_lowsb_up_err = 0.;
  double muon_lowsb_up_count = h_muon_jes_up_dijet_m->IntegralAndError(1,lowsb_bin,muon_lowsb_up_err);

  double muon_lowsb_down_err = 0.;
  double muon_lowsb_down_count = h_muon_jes_down_dijet_m->IntegralAndError(1,lowsb_bin,muon_lowsb_down_err);

  double muon_highsb_nom_err = 0.;
  double muon_highsb_nom_count = h_muon_jes_nom_dijet_m->IntegralAndError(highsb_bin,tot_bins,muon_highsb_nom_err);

  double muon_highsb_up_err = 0.;
  double muon_highsb_up_count = h_muon_jes_up_dijet_m->IntegralAndError(highsb_bin,tot_bins,muon_highsb_up_err);

  double muon_highsb_down_err = 0.;
  double muon_highsb_down_count = h_muon_jes_down_dijet_m->IntegralAndError(highsb_bin,tot_bins,muon_highsb_down_err);

  double muon_sig_nom_err = 0.;
  double muon_sig_nom_count = h_muon_jes_nom_dijet_m->IntegralAndError(lowsb_bin,highsb_bin,muon_sig_nom_err);

  double muon_sig_up_err = 0.;
  double muon_sig_up_count = h_muon_jes_up_dijet_m->IntegralAndError(lowsb_bin,highsb_bin,muon_sig_up_err);

  double muon_sig_down_err = 0.;
  double muon_sig_down_count = h_muon_jes_down_dijet_m->IntegralAndError(lowsb_bin,highsb_bin,muon_sig_down_err);
  ...
  TCanvas* tc_muon_jes_dijet_m = new TCanvas("tc_muon_jes_dijet_m","tc_muon_jes_dijet_m",0,0,1920,1200);
  tc_muon_jes_dijet_m->cd();

  h_muon_jes_up_dijet_m->SetLineColor(kRed);
  h_muon_jes_up_dijet_m->Rebin(2);
  h_muon_jes_up_dijet_m->Draw("hist");

  h_muon_jes_nom_dijet_m->Rebin(2);
  h_muon_jes_nom_dijet_m->Draw("hist same");

  h_muon_jes_down_dijet_m->Rebin(2);
  h_muon_jes_down_dijet_m->SetLineColor(kBlue);
  h_muon_jes_down_dijet_m->Draw("hist same");

  TLine comb_cut_line;
  TBox comb_cut_box;

  float comb_ymax = h_muon_jes_up_dijet_m->GetMaximum();
  comb_cut_line=TLine(65,0,65,comb_ymax*1.05);
  comb_cut_line.SetLineWidth(2);
  comb_cut_line.SetLineStyle(2);
  comb_cut_line.SetLineColor(kGray+3);
  ...
  double xmax = h_muon_jes_up_dijet_m->GetXaxis()->GetXmax();
  double xmin = h_muon_jes_up_dijet_m->GetXaxis()->GetXmin();
  ...
  tc_muon_jes_dijet_m->Update();
  TLegend* lgnd_muon_jes_dijet_m = new TLegend(0.80,0.70,0.9,0.90);
  lgnd_muon_jes_dijet_m->SetFillColor(0);
  lgnd_muon_jes_dijet_m->SetTextSize(0.04);
  lgnd_muon_jes_dijet_m->SetBorderSize(0);
  lgnd_muon_jes_dijet_m->AddEntry(h_muon_jes_nom_dijet_m,"Nominal","L");
  lgnd_muon_jes_dijet_m->AddEntry(h_muon_jes_up_dijet_m,"JES up","L");
  lgnd_muon_jes_dijet_m->AddEntry(h_muon_jes_down_dijet_m,"JES down","L");
  lgnd_muon_jes_dijet_m->Draw();
  
  cur_saveas = save_loc+"h_muon_jes_dijet_m.pdf";
  tc_muon_jes_dijet_m->SaveAs(cur_saveas.c_str());


  TCanvas* tc_elec_jes_ratio_dijet_m = new TCanvas("tc_elec_jes_ratio_dijet_m","tc_elec_jes_ratio_dijet_m",0,0,1920,1200);
  tc_elec_jes_ratio_dijet_m->cd();

  h_elec_jes_up_dijet_m->Rebin(5);
  h_elec_jes_down_dijet_m->Rebin(5);
  h_elec_jes_nom_dijet_m->Rebin(5);


  h_elec_jes_down_dijet_m->Divide(h_elec_jes_down_dijet_m,h_elec_jes_nom_dijet_m);
  h_elec_jes_down_dijet_m->SetLineColor(kBlue);
  h_elec_jes_down_dijet_m->GetYaxis()->SetRangeUser(0,1.7);
  h_elec_jes_down_dijet_m->GetXaxis()->SetTitle("M(jj) [GeV]");
  h_elec_jes_down_dijet_m->GetYaxis()->SetTitle("Fractional JES effect");
  
  h_elec_jes_down_dijet_m->Draw("hist E ");

  h_elec_jes_up_dijet_m->Divide(h_elec_jes_up_dijet_m,h_elec_jes_nom_dijet_m);
  //h_elec_jes_up_dijet_m->Rebin(2);
  h_elec_jes_up_dijet_m->SetLineColor(kRed);
  h_elec_jes_up_dijet_m->Draw("hist E same");

  
  TLine comb_cut_line;
  TBox comb_cut_box;

  float comb_ymax = h_elec_jes_down_dijet_m->GetMaximum();
  ...
  double xmax = h_elec_jes_down_dijet_m->GetXaxis()->GetXmax();
  double xmin = h_elec_jes_down_dijet_m->GetXaxis()->GetXmin();
  ...
  tc_elec_jes_ratio_dijet_m->Update();
  TLegend* lgnd_elec_jes_ratio_dijet_m = new TLegend(0.80,0.70,0.9,0.90);
  lgnd_elec_jes_ratio_dijet_m->SetFillColor(0);
  lgnd_elec_jes_ratio_dijet_m->SetTextSize(0.04);
  lgnd_elec_jes_ratio_dijet_m->SetBorderSize(0);
  lgnd_elec_jes_ratio_dijet_m->AddEntry(h_elec_jes_up_dijet_m,"JES up","L");
  lgnd_elec_jes_ratio_dijet_m->AddEntry(h_elec_jes_down_dijet_m,"JES down","L");
  lgnd_elec_jes_ratio_dijet_m->Draw();
  
  cur_saveas = save_loc+"h_elec_jes_ratio_dijet_m.pdf";
  tc_elec_jes_ratio_dijet_m->SaveAs(cur_saveas.c_str());
  
  TCanvas* tc_muon_jes_ratio_dijet_m = new TCanvas("tc_muon_jes_ratio_dijet_m","tc_muon_jes_ratio_dijet_m",0,0,1920,1200);
  tc_muon_jes_ratio_dijet_m->cd();

  h_muon_jes_up_dijet_m->Rebin(5);
  h_muon_jes_down_dijet_m->Rebin(5);
  h_muon_jes_nom_dijet_m->Rebin(5);


  h_muon_jes_down_dijet_m->Divide(h_muon_jes_down_dijet_m,h_muon_jes_nom_dijet_m);
  h_muon_jes_down_dijet_m->SetLineColor(kBlue);
  h_muon_jes_down_dijet_m->GetYaxis()->SetRangeUser(0,1.7);
  h_muon_jes_down_dijet_m->GetXaxis()->SetTitle("M(jj) [GeV]");
  h_muon_jes_down_dijet_m->GetYaxis()->SetTitle("Fractional JES effect");
  
  h_muon_jes_down_dijet_m->Draw("hist E ");

  h_muon_jes_up_dijet_m->Divide(h_muon_jes_up_dijet_m,h_muon_jes_nom_dijet_m);
  //h_muon_jes_up_dijet_m->Rebin(2);
  h_muon_jes_up_dijet_m->SetLineColor(kRed);
  h_muon_jes_up_dijet_m->Draw("hist E same");

  
  TLine comb_cut_line;
  TBox comb_cut_box;

  float comb_ymax = h_muon_jes_down_dijet_m->GetMaximum();
  comb_cut_line=TLine(65,0,65,comb_ymax);
  comb_cut_line.SetLineWidth(2);
  comb_cut_line.SetLineStyle(2);
  comb_cut_line.SetLineColor(kGray+3);
  ...
  double xmax = h_muon_jes_down_dijet_m->GetXaxis()->GetXmax();
  double xmin = h_muon_jes_down_dijet_m->GetXaxis()->GetXmin();
  ...
  tc_muon_jes_ratio_dijet_m->Update();
  TLegend* lgnd_muon_jes_ratio_dijet_m = new TLegend(0.80,0.70,0.9,0.90);
  lgnd_muon_jes_ratio_dijet_m->SetFillColor(0);
  lgnd_muon_jes_ratio_dijet_m->SetTextSize(0.04);
  lgnd_muon_jes_ratio_dijet_m->SetBorderSize(0);
  lgnd_muon_jes_ratio_dijet_m->AddEntry(h_muon_jes_up_dijet_m,"JES up","L");
  lgnd_muon_jes_ratio_dijet_m->AddEntry(h_muon_jes_down_dijet_m,"JES down","L");
  lgnd_muon_jes_ratio_dijet_m->Draw();
  
  cur_saveas = save_loc+"h_muon_jes_ratio_dijet_m.pdf";
  tc_muon_jes_ratio_dijet_m->SaveAs(cur_saveas.c_str());
