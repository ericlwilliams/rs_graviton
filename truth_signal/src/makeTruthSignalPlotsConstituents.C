#define makeTruthSignalPlotsConstituents_cxx

#include "makeTruthSignalPlotsConstituents.h"
#include <TH2.h>
#include <TH1.h>
#include <TStyle.h>
...
void makeTruthSignalPlotsConstituents::Begin(TTree * /*tree*/)
{
...
void makeTruthSignalPlotsConstituents::SlaveBegin(TTree * /*tree*/)
{
...
Bool_t makeTruthSignalPlotsConstituents::Process(Long64_t entry)
{
  ...
  b_jet_AntiKt4Truth_phi->GetEntry(entry);
  ...
  if(nu_n==1 && lep_n==1 && jet_n==2){//e/mu + nu + 2 quarks
  ...
  for(int j=0;j<jet_AntiKt4Truth_n;j++){
    ...
    double jet_lep_deta = (*jet_AntiKt4Truth_eta)[j] - lep.Eta();
    double jet_lep_dphi =
      TVector2::Phi_mpi_pi((*jet_AntiKt4Truth_phi)[j] - lep.Phi());

    double jet_lep_dr = sqrt(jet_lep_deta*jet_lep_deta + jet_lep_dphi*jet_lep_dphi);
    if(jet_lep_dr<0.4) continue;
  ...
  nu.SetPtEtaPhiM((*mc_pt)[nu_idx],
    ...
    //Cosine theta calc.
    // want v_star to be positive boson
    // if G*, find positive W,
    // if W', use lv


    TLorentzVector v_star = ((*mc_pdgId)[lep_idx]>0) ? lv : dijet;
...
void makeTruthSignalPlotsConstituents::SlaveTerminate()
{
...
void makeTruthSignalPlotsConstituents::Terminate()
{
