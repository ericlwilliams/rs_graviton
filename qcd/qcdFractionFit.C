...
vector<std::pair<float,float>>* qcdFractionFit(string generator="alpgen",string template_distribution="lep_met_mt"){
  ...
  string data_elec_hist = cut_dir+"/h_elec_"+template_distribution;
  TH1F* data_elec = (TH1F*) data_file->Get(data_elec_hist.c_str());
  string data_muon_hist = cut_dir+"/h_muon_"+template_distribution;
  ...
  // string wjets_contamination_file_loc = "../plots/gww_no_calo_iso_plots/merged/mc.alpgen.wjets.plot.root";
  // TFile* wjets_contamination_file = (TFile*) TFile::Open(wjets_contamination_file_loc.c_str());

  // string elec_wjets_contamination_name = cut_dir+"/h_elec_"+template_distribution;
  // TH1F* elec_wjets_contamination = wjets_contamination_file->Get(elec_wjets_contamination_name.c_str());
  // elec_wjets_contamination->SetDirectory(0);
  // wjets_contamination_file->Close();
  ...
  for(int s=0;s<nBkgds;s++){
    ...
    string elec_hist_name = cut_dir+"/h_elec_"+template_distribution;
    TH1F* elec_hist = (TH1F*) tmpFile->Get(elec_hist_name.c_str());
    elec_hist->SetDirectory(0);
    string muon_hist_name = cut_dir+"/h_muon_"+template_distribution;
    ...
    cout<<"adding " <<cur_bkgd<<", "<<elec_hist_name<<endl;
  ...
  cout<<"qcd: "<<elec_qcd->Integral()<<" vjets: "<<elec_vjets->Integral();
  cout<<" bkgd: "<<elec_backgrounds->Integral()<<" data: "<<data_elec->Integral()<<endl;
  ...
  fit_elec->Constrain(1,0.0,2.0);
  fit_elec->Constrain(2,0.0,2.0);
  fit_elec->Constrain(3,fraction_elec_other-0.00001,fraction_elec_other+0.00001);

  if(template_distribution=="lep_pt"){
    fit_elec->SetRangeX(4,12);
  }else if(template_distribution=="met"){
    fit_elec->SetRangeX(3,12);
  }else if(template_distribution=="lt"){
    fit_elec->SetRangeX(6,8);
  }else{
    fit_elec->SetRangeX(1,10);
  }
  ...
  fit_muon->Constrain(1,0.0,2.0);
  fit_muon->Constrain(2,0.0,2.0);
  fit_muon->Constrain(3,fraction_muon_other-0.00001,fraction_muon_other+0.00001);

  if(template_distribution=="lep_pt"){
    fit_muon->SetRangeX(4,22);
  }else if(template_distribution=="met"){
    fit_muon->SetRangeX(3,10);
  }else if(template_distribution=="lt"){
    fit_muon->SetRangeX(5,7);
  }else{
    fit_muon->SetRangeX(1,10);
  }
